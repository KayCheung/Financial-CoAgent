...
> **版本**：v1.1 
> **上游基座**：基于 DeerFlow 的企业级 Agent 平台完整方案v1.6.md  
...
5. [Web Search（受控实时检索）](#5-web-search受控实时检索)
   - 5.1 合规前置：金融场景网络访问约束
   - 5.2 检索工具架构
   - 5.3 沙箱隔离：Playwright 专属 Profile
   - 5.4 结果处理：可信度打分与缓存
   - 5.5 检索结果进入 L3 记忆的策略
...
7. [持久化调度器（Scheduler）](#7-持久化调度器scheduler)
   - 7.1 调度器选型决策
   - 7.2 外部触发 LangGraph 接口
   - 7.3 TokenBudget 扩展：entity 级配额
   - 7.4 调度任务的失败补偿
...
| P1 | 协同（评论/共享/审批） | L2 事件源、HITL、ABAC、RocketMQ | 低（新增服务） | 是 |
...
2. **状态与审计可恢复硬约束**：新增的 Board 服务状态、调度任务状态、协同操作记录，全部纳入 WAL + RocketMQ 审计链路。
3. **发布准入硬约束**：二期每个新 Skill（尽调、Web Search）上线前，必须通过 Evaluation 回放门禁。
...
nodes:
  ...
  - id: n4_risk_score
    ...
    dependencies: [n2_financial_audit, n3_legal_check]
    timeout_s: 180
...
尽调报告存储策略
  ...
  │         └─ 异步 → RocketMQ → ClickHouse 永久留存（合规轨）
  │
  ├─ 最终尽调报告（PDF/DOCX）
...
ClickHouse 扩展 Schema（在一期 `agent_audit_hot` 基础上新增列族）：
...
CREATE TABLE dd_plan_audit (
    ...
    tenant_id    String,
    node_id      String,
    node_type    Enum('doc_collect','evidence_verify','external_fetch',
                      'risk_score','approval_gate','report_gen'),
    input_hash   String,         -- 输入参数 SHA256（不存原文，防敏感信息泄露）
    output_uri   String,         -- OSS URI（Claim-Check）
    risk_score   Nullable(Float32),
    rule_hits    Array(String),  -- 命中规则 ID 列表
    duration_ms  UInt32,
    status       Enum('success','failed','hitl_pending','blocked'),
    operator_id  String,
    ts           DateTime
) ENGINE = MergeTree()
PARTITION BY toYYYYMM(ts)
ORDER BY (tenant_id, plan_id, ts)
TTL ts + INTERVAL 7 YEAR;    -- 合规留存 7 年
```
...
class CommentService:
    """
    评论服务。
    写入走 WAL + RocketMQ → ClickHouse 审计轨，不走 L2 事件源（避免污染 Agent 状态流）。
    """
...
class WebSearchTool:
    ...
    不支持 headless 浏览器（阶段一）；headless 浏览器沙箱在阶段二评估。
    """
...
### 5.3 沙箱隔离：Playwright 专属 Profile（阶段二评估）

阶段一使用 Search API（Brave/Bing），不启用 headless 浏览器。以下为阶段二的 Playwright 沙箱设计预案，供架构评审：

```yaml
# 阶段二预案：Playwright headless 沙箱 Profile（不在二期交付）
...
class KanbanEventConsumer:
    """
    RocketMQ 消费者：消费一期 agent_audit_log 事件流，更新看板卡片状态。
    消费位置：RocketMQ agent_audit_log topic（一期已有）。
    不修改任何一期组件，纯粹的读侧消费。
    """
...
### 7.4.1 调度任务成本聚合视图（ClickHouse）

```sql
-- 二期新增：按 entity 聚合 Token 消耗，支持调度任务成本核算
CREATE MATERIALIZED VIEW scheduler_cost_summary
ENGINE = SummingMergeTree()
PARTITION BY toYYYYMM(ts)
ORDER BY (tenant_id, entity_type, entity_id, toDate(ts))
AS SELECT
    tenant_id,
    entity_type,
    entity_id,
...
GROUP BY tenant_id, entity_type, entity_id, toStartOfDay(ts);
```
...
| 看板事件消费延迟 P99 | > 5s | RocketMQ 消费滞后或看板服务性能问题 |
...
| GPU | 2 | 0（二期不引入多模态） | 2 |
| 新增 PG 表 | - | 8 张（见 §6.1、§4.2、§7.2） | - |
| 新增 ClickHouse 表 | - | 2 张（见 §3.5、§7.4.1） | - |
...
P2-M6  看板 + 调度器
  ...
  ├─ KanbanEventConsumer（RocketMQ 消费 L2 事件）
