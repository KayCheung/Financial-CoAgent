...
| 阶段 | 周期 | 核心目标 | 可见成果 |
|------|------|----------|----------|
| **一阶段：尽调基础版** | P2-M1-M2（8周） | 金融尽调 Plan 模式 + 受控多 Agent 入口 | 企业/个人尽调可发起、评分报告（JSON）可输出 |
| **二阶段：尽调深水区 + 协同** | P2-M3-M4（8周） | 外部数据接入 + 协同审批 | 外部工商数据打通、多人协同审批可用、报告可生成 |
...
**让业务用户看到可交互的尽调成果**：能发起企业/个人尽调 Plan、触发受控多 Agent 执行、输出风险评分（JSON）。
...
| 报告完整度 | DDReflectionGate 报告检查点单元测试通过，覆盖四章缺失、关键字段为空两类场景 |
...
**类型**：弱依赖 T2.2（集成时需要 Plan 对象模型完成后接入 Gateway）  
**预估工时**：4天
...
**验收标准**：从对话入口分别发起企业尽调和个人尽调，企业尽调主体类型校验通过，Harness 策略允许 web_search 工具槽位（工具本身在三阶段上线）；个人尽调触发该槽位时被 Harness 拒绝并返回明确错误
...
- 模板 YAML 可与 T2.3 并行编写（风险评分节点填占位 skill_id），但 Skill 上架审核必须在 T2.3 完成并通过 Evaluation 基线后才能执行
...
| 任务 | 前置依赖 | 依赖类型 | 说明 |
|------|---------|---------|------|
| T2.1 受控多 Agent 入口 | T2.2 | 弱依赖 | 集成需 Plan 模型 |
| T2.2 DAG 编排 + Reflection | 无 | — | 独立任务 |
| T2.3 评分引擎 | T2.2 | 接口依赖 | 需要证据包结构 |
| T2.4 模板引擎 + Skill 上架 | 无 | — | 独立任务 |
...
| RocketMQ/ONS（已有） | 协同事件分发 | 复用一期（须确认一期生产就绪） | 0 |

> **阶段二资源增量**：约 +2C +4G
>
> **前置要求**：需确认一期 RocketMQ/ONS 已完成生产环境接入（对应一期 T3.2 WAL Janitor 升级）；若未完成，二期 M1 前需补做接入。
...
| 调度 Job 持续 `FAILED_CLOSED` 触发自动禁用 | FAILED_CLOSED | Job 已写 DLQ，需人工介入后才可重新启用 |
