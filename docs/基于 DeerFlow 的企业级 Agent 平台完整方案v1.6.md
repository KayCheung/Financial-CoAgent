...
触发: CFO 提交「Q3 财务审计」任务
  ...
  │           推送飞书审批卡片 → 财务总监
  │           按风险等级催办（urgent 固定提前 15min）→ 财务总监批准 [附注: 特批]
  │           补偿事务: 已释放预占沙箱资源
  │
  ├─[记忆层] 决策指纹 → L2 PG + WAL (RocksDB)
  │           异步 → RocketMQ → ClickHouse 永久留存
  │
...
class HarnessScratchpad:
    ...
    def __init__(self, thread_id: str, rule_engine, audit_queue, redis_client):
        ...
        self._step_counter_key = f"scratchpad:{thread_id}:step_counter"
        self._shard_index_key = f"scratchpad:{thread_id}:shard_index"
    ...
    async def write(self, step: str, content: str) -> None:
        ...
        # 步号在通过安全校验后才生成，避免违规请求污染 L1 序列。
        step_idx = await self._redis.incr(self._step_counter_key)
        ...
        # 分片写入和索引登记放进同一批次，降低 step_idx、内容、索引错位概率。
        pipe = self._redis.pipeline()
        pipe.setex(shard_key, 86400, shard_data)
        pipe.rpush(self._shard_index_key, shard_key)
        pipe.expire(self._shard_index_key, 86400)
        await pipe.execute()
...
DEFAULT_FAILURE_REGISTRY = {
    "TimeoutError": AgentFailureState.RETRYABLE,
    "NetworkJitterError": AgentFailureState.RETRYABLE,
    "ResourceContentionError": AgentFailureState.RETRYABLE,
    "InvalidOCRPayloadError": AgentFailureState.HUMAN_REVIEW,
    "ApprovalTimeoutError": AgentFailureState.HUMAN_REVIEW,
    "UntrustedModelOutputError": AgentFailureState.HUMAN_REVIEW,
    "TokenBudgetExhaustedException": AgentFailureState.FAILED_CLOSED,
    "PermissionDeniedError": AgentFailureState.FAILED_CLOSED,
    "ComplianceRedlineTriggered": AgentFailureState.FAILED_CLOSED,
}


def map_exception_to_failure_state(exc: Exception) -> AgentFailureState:
    # 节点内部不自行发明终态，统一通过注册表归类。
    exc_name = type(exc).__name__
    return DEFAULT_FAILURE_REGISTRY.get(exc_name, AgentFailureState.FAILED_CLOSED)


async def run_executor_node(node, context):
    try:
        return await node.execute(context)
    except Exception as exc:
        failure_state = map_exception_to_failure_state(exc)
        await context.audit_logger.log_exception(
            node_name=node.name,
            exception_type=type(exc).__name__,
            mapped_state=failure_state.value,
            registry_version="failure-registry.v1",
        )
        raise AgentNodeFailed(failure_state=failure_state, cause=exc) from exc
```
...
class StateCheckpoint:
    # 快照保存“某个 event_offset 之后可继续回放”的恢复基线。
    thread_id: str
    schema_version: int
    event_offset: int
    created_at: float
...
async def append_state_event(
    ...
    event = StateEvent(
        ...
        step_id=step_id,
        event_offset=await pg_store.next_event_offset(thread_id),
        ts=time.time(),
        payload={
    ...
    # 关键阶段或事件数达到阈值时生成检查点，避免长线程恢复退化为全量回放。
    if await pg_store.should_checkpoint(thread_id, latest_offset=event.event_offset):
        ...
        await pg_store.save_checkpoint(asdict(checkpoint))
...
async def rebuild_thread_state(thread_id: str, redis_client, pg_store) -> dict:
    ...
    checkpoint = await pg_store.get_latest_checkpoint(thread_id)
    if checkpoint:
        rebuilt = checkpoint["snapshot"]
        start_offset = checkpoint["event_offset"]
    else:
        rebuilt = {
            "schema_version": STATE_SCHEMA_VERSION,
            "thread_id": thread_id,
    ...
    events: List[dict] = await pg_store.list_events_after(thread_id, start_offset)
    if not events:
        return rebuilt if checkpoint else {"status": "NOT_FOUND", "thread_id": thread_id}
...
class ApprovalRecord:
    ...
    @property
    def remind_lead_seconds(self) -> int:
        # 紧急审批用绝对提前量，避免 1h 审批被压缩到只剩几分钟才提醒。
        return {
            "normal": 21600,   # 提前 6h
            "high": 3600,      # 提前 1h
            "urgent": 900,     # 提前 15min
        }.get(self.risk_level, 21600)
...
class HITLApprovalStateMachine:
    ...
    async def submit(self, record: ApprovalRecord) -> str:
      ...
      async def wait_for_decision(self, approval_id: str, record: ApprovalRecord) -> bool:
          ...
          # 换算成“距离超时还剩多久时提醒”，并保留最小缓冲，避免提交即提醒。
          remind_at = max(timeout - record.remind_lead_seconds, 60)
          ...
          async def _remind_task():
              ...
              if latest.status == ApprovalStatus.PENDING:
                  remaining_seconds = max(timeout - remind_at, 0)
                  remaining_hint = (
                      f"{remaining_seconds // 60}min"
                      if remaining_seconds < 3600
                      else f"{remaining_seconds / 3600:.1f}h"
                  )
                  await self._im.send_reminder(
                      ...
                      message=(
                          ...
                          f"将在 {remaining_hint} 后超时自动否决，请及时处理。"
                      ),
                  )
...
本节参数不再视为“拍脑袋默认值”。冷启动阈值、影子验证窗口与退出条件，必须来自首批种子语料的离线评测与试运行样本统计。若租户向量库为空、样本量低于最小种子规模，Router 不得伪装成“正常冷启动”，而应直接进入 `LLM fallback + 人工标注采样` 模式。
...
class RouterConfig:
    """路由配置（支持 Nacos 热更新）"""
    threshold_high:        float = 0.90    # 来源: router_eval_report.v1 的稳定期 high precision cutoff
    threshold_low:         float = 0.60    # 来源: router_eval_report.v1 的兜底切换下限
    shadow_sample_rate:    float = 0.05    # 稳定期抽样率
    is_cold_start:         bool  = True    # 冷启动标志（前 1000 次请求）
    cold_start_threshold_high: float = 0.75  # 来源: 首批种子集 recall 优先实验结果
    ...
    min_seed_samples: int = 200              # 启用向量直通前的最小种子样本量
    refresh_interval_requests: int = 20      # 每 N 次请求刷新一次冷启动状态
    eval_report_version: str = "router_eval_report.v1"
...
class AdaptiveSemanticRouter:
    ...
    async def route(self, query: str, tenant_id: str) -> str:
        with tracer.start_as_current_span("SemanticRouter.route") as span:
            ...
            collection_size = await self._vdb.count(tenant_id=tenant_id)

              # 向量库样本不足时不做“伪高置信”路由，直接走 LLM 兜底并沉淀标注样本。
              if collection_size < self.config.min_seed_samples:
                  span.set_attribute("router.seed_insufficient", True)
                span.set_attribute("router.eval_report_version", self.config.eval_report_version)
                asyncio.create_task(
                    self._finetune_q.put({
                        "query": query,
                        "tenant_id": tenant_id,
...
本节遵循“本地先落盘、网络后异步”的审计原则，确保 RocketMQ 或网络异常时主链路不丢日志。  
...
async def secure_audit_flow(entry: dict, wal: LocalWALStore, rocket_producer) -> None:
    """
    双写审计主流程:
    1. 同步本地 WAL 落盘（优先，保障不丢）
    2. 异步发送 RocketMQ（解耦主链路延迟）
    """
    ...
    # 第二步: 异步 RocketMQ 发送（不阻塞主链路）
    asyncio.create_task(_send_to_rocket(entry_id, entry, wal, rocket_producer))


async def _send_to_rocket(entry_id, entry, wal, rocket_producer) -> None:
    try:
        await rocket_producer.send("agent_audit_log", entry)
        await wal.mark_sent(entry_id)
    except Exception as e:
        logger.error(f"RocketMQ 发送失败，WAL 保留待 Janitor 重试: {entry_id}: {e}")


# ── Janitor 守护进程（独立 DaemonSet Pod）──
async def janitor_main(wal: LocalWALStore, rocket_producer, check_interval: int = 300) -> None:
    """
    ...
    async def tokenize(self, text: str, session_id: str) -> str:
        ...
        cursor = 0
          for start, end, pii_type, original in merged:
              ...
              # token_map 记录 token 到原文的精确映射，回填时不再依赖模糊匹配。
              token_map[token] = {
                "value": original,
                "start": start,
                "end": end,
...
| WAL 未同步条目数 | > 100 | RocketMQ 网络问题或 Janitor 故障 |
...
| **容灾/审计** | RocksDB(WAL) + ClickHouse | RocketMQ（AliyunOns） 双写防止审计数据丢失            |
...
M1-M2  基座搭建
  ...
  ├─ 基础 RBAC 上线
  └─ WAL 双写审计基础版（本地 WAL + RocketMQ）

M3-M4  首个闭环与路由冷启动
  ├─ Semantic Router 种子语料建设 + 参数基线评测
  ├─ Semantic Router 冷启动版（含空库兜底与影子验证）
  ├─ 知识库 Agent（第一个生产 Skill，Beta）
  ├─ PII 双向脱敏网关
  └─ Evaluation 首版基线（标准样例集 + 脱敏真实流量）

M5-M6  Harness 构建
  ├─ HarnessToolInterceptor 完整版（含 Nacos 热更新 + 灰度）
  ├─ Scratchpad 分片压缩审计 + ClickHouse 审计消费者
  ├─ 三层记忆（L1 分片 + L2 Checkpoint + Event Diffs + L3 租户隔离）
  ├─ HITL 三段式状态机（含资源补偿事务）
  ├─ 统一失败态映射表接入 Planner/Executor/Critic
...
M9  平台化运营
  ...
  ├─ 二级路由（防 Skill 路由过载）
  ├─ Token 消耗租户维度监控 + 成本告警
  ├─ Evaluation Layer 扩展（真实 Workflow 回放 + Prompt 回归）
...
为避免平台在落地过程中重新滑向“控制面无限堆叠”或“实现细节失控”，本方案将实施约束显式拆为三类：`不可协商的合规/生存底线`、`首期允许的简化实现`、`实施治理要求`。实现者不得在压力下自行解释约束级别。
...
| 审计可靠性 | WAL 先写本地 + RocketMQ 异步 | 直接写 RocketMQ | 网络故障不丢日志 |
