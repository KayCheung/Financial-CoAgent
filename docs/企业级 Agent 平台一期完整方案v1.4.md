...
| 能力标签 | 含义 | 典型适用节点 |
|---|---|---|
| `structured_output_strict` | 严格输出 JSON / Schema，不漂移字段 | Planner、Critic、记忆脱水 |
...
本节将权限校验、参数约束、风险嗅探与规则版本绑定到统一工具入口，使工具调用具备可拦截、可灰度、可追溯属性。
对于写操作工具，默认要求支持 `dry-run` 预演模式：先返回影响范围、对象清单与变更预览，经确认后再进入真实执行。
同时，Agent 所使用的操作账号必须遵循最小权限原则，权限申请与提权变更均通过内部审批流程完成，不依赖模型自身承诺。
...
本节将长链路过程压缩为可恢复、可校验、可迁移的结构化状态，因此强调 Schema 版本化、校验重试与失败保底。

在实现形态上，L2 由两部分组成：

1. 关键实体记忆：通过 NER / 规则抽取时间、地点、人物、任务、票据编号、审批对象等关键实体，写入结构化 KV 或图谱化关系。
2. 周期性摘要记忆：每 10 轮对话或每个关键阶段结束后，生成一段结构化摘要，描述“用户目标、当前进展、未完成事项、主要障碍”。

这样设计的目的，是让 L2 同时承担“事实记忆”与“进度记忆”，避免只有摘要没有实体，或只有实体没有任务推进状态。
...
#### 4.4.4 Redis / MinIO / Claim-Check

| 组件 | 默认隔离策略 | 高敏升级策略 |
|---|---|---|
| Redis | key namespace 带 `tenant_id` 前缀 | 独立 DB / 独立实例 |
| MinIO | 按租户前缀或 Bucket 隔离 | 独立 Bucket + KMS 租户密钥 |
...
Semantic Router 的目标不是“永远命中最优 Skill”，而是在错误不可避免时，将错误收敛为“可观测、可回退、可人工校正”的安全行为。
...
本节通过预热池、排队透传与超时返回三项机制，把高并发场景下的沙箱容量问题显式暴露给上层，而不是在底层静默堆积。
...
| 场景 | 风险 | 处理策略 |
|---|---|---|
| Pod 创建成功但未 Ready | 线程拿到不可用执行环境 | 引入 `SandboxAllocationRecord`，仅在 Ready 后确认分配成功 |
| Pod 已分配但线程取消 | 资源悬挂，池子被击穿 | 由回收器按分配记录表兜底回收 |
| Redis 队列状态与 K8s 实际状态不一致 | 资源统计失真 | 以分配记录表与 K8s 实际状态为准，Redis 仅作加速层 |
| Release 失败 | 脏环境残留 | 标记 `PENDING_RECYCLE`，Janitor 异步清理 |
| 租户级资源配额耗尽 | 单租户挤占公共资源 | 按租户设置并发上限与硬配额 |
| 预热池补充失败 | 下一波请求抖动放大 | 触发容量告警，允许短时退化为动态创建 |

#### 规则补充

1. 沙箱资源生命周期必须落分配记录表，不以 Redis 队列为真相源
2. 释放失败不允许静默吞掉，必须进入待回收状态
3. 对高风险 Skill，线程取消时必须优先执行资源回收
4. 预热池的目标是削峰，不是状态真源；容量与一致性审计以 K8s 事实为准
...
| Skill 下架 | 历史线程无法重放 | 历史线程继续绑定旧版本，只对新流量下线 |
...
Skill 治理不仅是“上架审核”，还必须包含“灰度、回滚、重放兼容、依赖升级影响分析”四个闭环能力。
