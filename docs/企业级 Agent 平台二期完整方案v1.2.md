...
| RocketMQ/ONS 生产就绪 | 对应一期 T3.2 接入完成，支持 WAL Janitor 重放 | 安全/运维 |
...
- **企业尽调（`ENTERPRISE`）**：允许访问合规白名单内的 Web Search 与工商、司法、政务等公开数据源，适用于企业财务、法务、税务、合规等多维核查（注：Web Search 工具槽位在二期 M1 开放，工具服务在三期交付）。
...
PHASE2_FAILURE_REGISTRY = {
    ...
    # 调度专项
    "ScheduleJobDisabledError":    AgentFailureState.FAILED_CLOSED,  # 调度 Job 持续 FAILED_CLOSED 触发自动禁用
